...
import { ColumnComponent, FormatAmountPipe, RowResumenComponent, TableComponent, TruncatePipe, SkeletonTableComponent, StatusStateComponent, StatusStateEnum } from "intelica-library-project";
import { FeeLibraryFilter } from "../DTO/request";
import { FeeRefund } from "../DTO/response";
...
@Component({
	selector: "fee-modal-allocated-refund",
	imports: [CommonModule, ButtonModule, TooltipModule, SkeletonModule, TableComponent, ColumnComponent, RowResumenComponent, TruncatePipe, FormatAmountPipe, StatusStateComponent],
	templateUrl: "./modal-allocated-refund.html",
	styleUrl: "./modal-allocated-refund.css",
	providers: [DatePipe],
})
export class ModalAllocatedRefund {
	...
	readonly listData = signal<FeeRefundRow[]>([]);
	readonly showTable = signal(true);
	readonly itltable = viewChild<TableComponent<FeeRefundRow>>("itltable");
	searchText = "";
	...
	private loadData(filter: FeeLibraryFilter): void {
		...
		this.libraryService.getFeeRefund(filter).subscribe({
			next: data => {
	...
	private mapRow(r: FeeRefund): FeeRefundRow {
		return {
			id: r.id,
	...
	private buildExportFileName(): string {
		return this.commonService.buildExportFileName(this.commonGlobalService.termText(this.libraryTerm.MODULE_EXPORT), this.commonGlobalService.termText(this.libraryTerm.NAME_REFUND_EXPORT));
	}
	...
	readonly filteredListData = signal<FeeRefundRow[]>([]);
	...
	onFilteredDataChange(data: FeeRefundRow[]): void {
	...
	get searchFields(): string[] {
		const fields = [this.commonGlobalService.termText(this.libraryTerm.DATE), this.commonGlobalService.termText(this.libraryTerm.BRAND)];
