...
export class AlertsDetailNewFeesComponent {
	...
	showAllFees = signal(false);
	private readonly initialCount = 3;

	visibleFees = computed(() => {
		const fees = this.data()?.fees ?? [];
		return this.showAllFees() ? fees : fees.slice(0, this.initialCount);
	});
